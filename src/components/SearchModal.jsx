...
function SearchModalPanel({ onClose }) {
  ...
  return (
    <motion.div
      className="fixed inset-0 z-[60] flex items-start justify-center px-3 pt-24 sm:pt-28"
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
    >
      ...
      >
        <div className="flex items-center gap-2 border-b border-slate-100 px-3 py-2 dark:border-slate-800">
          ...
          <input
            ...
            className="flex-1 border-0 bg-transparent py-2 text-sm text-slate-900 outline-none placeholder:text-slate-400 dark:text-white"
          />
          <button
            type="button"
            onClick={onClose}
            className="rounded-full p-2 text-slate-500 transition hover:bg-slate-100 hover:text-slate-800 dark:hover:bg-slate-800 dark:hover:text-white"
            aria-label="Close"
          >
            <FiX className="text-lg" />
          </button>
        </div>
        <ul className="max-h-[min(60vh,420px)] divide-y divide-slate-100 overflow-y-auto dark:divide-slate-800">
          {filtered.length === 0 ? (
            ...
            filtered.map((item) => (
              <li key={item.id}>
                <button
                  ...
                  className="flex w-full flex-col gap-1 px-4 py-3 text-left text-sm transition hover:bg-brand-50 dark:hover:bg-slate-800/80"
                >
                  ...
                  {item.subtitle ? (
                    <span className="text-xs text-slate-600 dark:text-slate-400">{item.subtitle}</span>
                  ) : null}
                </button>
              </li>
            ))
